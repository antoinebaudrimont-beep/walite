...
func NewSendTextRequest(chatID, text string) (SendTextRequest, error) {
	...
	return SendTextRequest{chatID: ownedChatID, text: strings.Clone(text)}, nil
}
...
func (core *Core) SendText(ctx context.Context, request SendTextRequest) error {
	...
	owned, err := NewSendTextRequest(request.ChatID().String(), request.Text())
	if err != nil {
		return err
	}
	...
	event, err := core.sender.SendText(ctx, owned.ChatID(), owned.Text())
	if err != nil {
