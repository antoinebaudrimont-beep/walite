...
func TestWideNarrowShortWidePreservesInteractiveState(t *testing.T) {
	...
	model.replyTarget = replyTarget{valid: true, id: model.chats[2].messages[5].id}
	...
	wantChat := model.chats[2].title
	wantMessageID := model.chats[2].messages[5].id
	wantTarget := model.replyTarget
	for _, size := range [][2]int{{120, 40}, {60, 20}, {30, 6}, {120, 40}} {
		clampView(&model, size[0], size[1])
		if model.chats[model.selectedChat].title != wantChat {
			t.Fatalf("%dx%d selected chat=%q", size[0], size[1], model.chats[model.selectedChat].title)
		}
		if !model.replySelect.valid || model.chats[model.selectedChat].messages[model.replySelect.index].id != wantMessageID {
