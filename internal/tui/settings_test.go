...
func TestNavigationFooterIncludesSettingsShortcut(t *testing.T) {
	model := defaultDemoView()
	want := "↑/↓ scroll  j/k chats  O older  P/S media  F file  Enter compose  Ctrl-P settings  Esc quit"
...
func TestPolishedFooterModesAndNarrowFallbacks(t *testing.T) {
	model := defaultDemoView()
	if got := navigationFooter(&model, false); got != "↑/↓ scroll  j/k chats  O older  P/S media  F file  Enter compose  Ctrl-P settings  Esc quit" {
	...
	if got := navigationFooter(&model, false); got != "↑/↓ message  L link  R react  P preview  S save  Enter reply  Esc cancel" {
