...
func TestAsyncSendProtectsDraftCursorReplyAndViewport(t *testing.T) {
	...
	if calls != 1 || model.composer != beforeDraft || model.replyTarget != beforeReply || model.chatView.scrollOffset != 2 || *model.chats != beforeChat {
		t.Fatal("pending send modified protected state or retried")
	}
	if !applySendResult(&model, SendResult{Failed: true}) || model.sendPending || model.sendUncertain || model.composer != beforeDraft || model.replyTarget != beforeReply || model.chatView.scrollOffset != 2 || *model.chats != beforeChat {
...
func TestAsyncSendSuccessWaitsForLiveMessageAndEchoIsIdempotent(t *testing.T) {
	...
	if *model.chats != before || model.composer.length != 0 || model.chatView.scrollOffset != 0 {
...
func TestAsyncPreflightRejectionKeepsStateAndShowsStatus(t *testing.T) {
	...
	if !submitOutgoingMessage(&model) || model.sendPending || model.composer != beforeDraft || model.replyTarget != beforeReply || *model.chats != beforeChats || model.chatView.scrollOffset != 2 || model.sendStatus == "" {
