...
func TestMilestone4BProductionCompositionUsesConnectionOwnedSourceAndSender(t *testing.T) {
	...
	if !strings.Contains(string(application), "connection.RealtimeSource()") ||
		...
		!strings.Contains(string(application), "connection.HistoryRequester()") ||
		!strings.Contains(string(application), "realtimeSource.SeedChatIDs(cachedIDs)") ||
		!strings.Contains(string(application), "newConnectedApplicationServiceWithCapabilities(realtimeSource, textSender, readReceiptSender, mediaDownloader, historyRequester, cache)") {
